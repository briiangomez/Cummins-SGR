...
namespace CMM.Web.Mvc.Grid
{
    public class GridModel
    {
        ...
        protected virtual void Initialize(ModelMetadata modelMetadata, IEnumerable dataSource, ViewContext viewContext)
        {
            ...
            {
                var gridAttribute = ((CMMModelMetadata)modelMetadata).Attributes.OfType<GridAttribute>().FirstOrDefault();
            ...
            PageSizeDifinition = ((CMMModelMetadata)modelMetadata).Attributes.OfType<PageSizeAttribute>().FirstOrDefault();
        ...
        protected virtual IEnumerable<GridAction> GetActions(ModelMetadata modelMetadata, ViewContext viewContext)
        {
            ...
            if (modelMetadata is CMMModelMetadata)
            {
                foreach (var item in ((CMMModelMetadata)modelMetadata).Attributes.OfType<GridActionAttribute>().OrderBy(it => it.Order))
                {
        ...
        protected virtual IEnumerable<GridColumn> GetGridColumns(ModelMetadata propertyMetadata)
        {
            if (ModelMetadata is CMMModelMetadata)
            {
                foreach (var att in ((CMMModelMetadata)propertyMetadata).Attributes.OfType<GridColumnAttribute>())
                {
        ...
        protected virtual IEnumerable<GridCommandAttribute> GetGridCommandDefinitions(ModelMetadata modelMetadata)
        {
            if (modelMetadata is CMMModelMetadata)
            {
                return ((CMMModelMetadata)modelMetadata).Attributes.OfType<GridCommandAttribute>().OrderBy(it => it.Order).ToArray();
            }
            else
            {
