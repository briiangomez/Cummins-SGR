namespace CMM.Web.Mvc.Paging
{
    ...
    public class PagerBuilder
    {
        ...
        public PagerBuilder(HtmlHelper html, AjaxHelper ajax, string actionName, string controllerName, int totalPageCount, int pageIndex, PagerOptions pagerOptions, string routeName, RouteValueDictionary routeValues, AjaxOptions ajaxOptions, IDictionary<string, object> htmlAttributes)
        {
            this._totalPageCount = 1;
            this._startPageIndex = 1;
            this._endPageIndex = 1;
        ...
        private string GenerateAnchor(PagerItem item)
        {
            ...
            if ((!string.IsNullOrEmpty(this._ajaxOptions.OnFailure) || !string.IsNullOrEmpty(this._ajaxOptions.OnBegin)) || (!string.IsNullOrEmpty(this._ajaxOptions.OnComplete) && (this._ajaxOptions.HttpMethod.ToUpper() != "GET")))
            {
                ...
                builder.Append("',url:$(this).attr('href'),success:function(data,status,xhr){$('#");
            ...
            return (string.IsNullOrEmpty(str) ? this._html.Encode(item.Text) : string.Format(CultureInfo.InvariantCulture, "<a href=\"{0}\" onclick=\"{1}\">{2}</a>", new object[] { str, builder, item.Text }));
        }
        ...
        private MvcHtmlString GenerateJqAjaxPagerElement(PagerItem item)
        {
            if (item.Disabled)
            {
                return this.CreateWrappedPagerElement(item, string.Format("<a disabled=\"disabled\" class=\"disabled\">{0}</a>", item.Text));
            }
        ...
        protected virtual string GenerateUrl(int pageIndex)
        {
            ...
            RouteValueDictionary routeValues = this._routeValues ?? new RouteValueDictionary();
            if (pageIndex == 0)
            {
        ...
        public MvcHtmlString RenderPager()
        {
            ...
            else if (this._pagerOptions.UseJqueryAjax)
            {
                foreach (PagerItem item in results)
                {
                    builder.Append(this.GenerateJqAjaxPagerElement(item));
                }
            }
            else
            {
                foreach (PagerItem item in results)
                {
                    builder.Append(this.GeneratePagerElement(item));
                }
            }
