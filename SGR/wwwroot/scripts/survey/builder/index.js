yardi.builderClass = function () {
    ...
    var targetIframe = function () {
        ...
        // Uso del valor posiblemente no asignado. NULL
        return frm;
    }();

    // tab
    var tabObject;
    var tabItems = [{ title: 'Componentes', width: 100, content: null },
                    { title: 'Propiedades', width: 100, content: null },
                    { title: 'Encuesta', width: 100, content: null }];
    ...
    var checkFieldEmptyMessage = function () {
        if (inputFieldsCount() == 0) {
            messageObject.show('¡No tiene componentes todavía!', 'Haga clic en los botones que están a su derecha para agregar componentes a la encuesta.');
            //actionBtns.hide();
        } else {
            messageObject.hide();
        }
    ...
    return {
        ...
        initializeSidebar: function () {
            ...
            var createItem = function (text, cls, type) {
                var btn = $('<div class="btnwrap"><div class="loc"><a href="javascript:;" class="locbtn">editar este campo</a>.</div><a href="javascript:;" class="addbtn ' + cls + '"><span>' + text + '</span></a></div>').appendTo(con);
            ...
            createItem('Texto', 'btnText', types.textBox);
            createItem('Etiqueta', 'btnTextLabel', types.textLabel);
            
            //createItem('TextArea', 'btnTextarea', types.textArea);
            //createItem('ComboBox', 'btnCombobox', types.comboBox);
            createItem('Lista Opciones', 'btnRadiolist', types.radioList);
            createItem('Lista Checks', 'btnChecklist', types.checkList);
            createItem('Escala Valores', 'btnRatescale', types.rateItems);
            createItem('PageBreak', 'btnPagebreak', types.pageBreak);
            createItem('Pie de página', 'btnCopyright', types.copyright);
