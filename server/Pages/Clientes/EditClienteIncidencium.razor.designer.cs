...
namespace Sgi.Pages
{
    public partial class EditClienteIncidenciumComponent : ComponentBase
    {
        ...
        {
            var sgiCoreGetClienteIncidenciumByIdResult = await SgiCore.GetClienteIncidenciumById(Id);
            clienteincidencium = sgiCoreGetClienteIncidenciumByIdResult;
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Sgi.Models.SgiCore.ClienteIncidencium args)
        {
            try
            {
                var sgiCoreUpdateClienteIncidenciumResult = await SgiCore.UpdateClienteIncidencium(Id, clienteincidencium);
                DialogService.Close(clienteincidencium);
            }
            catch (System.Exception sgiCoreUpdateClienteIncidenciumException)
            {
                    NotificationService.Notify(NotificationSeverity.Error, $"Error", $"Unable to update ClienteIncidencium");
            }
        }
