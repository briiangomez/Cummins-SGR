...
namespace Sgi.Pages
{
    public partial class EditFallaComponent : ComponentBase
    {
        ...
        {
            var sgiCoreGetFallaByIdResult = await SgiCore.GetFallaById(Id);
            falla = sgiCoreGetFallaByIdResult;
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Sgi.Models.SgiCore.Falla args)
        {
            try
            {
                var sgiCoreUpdateFallaResult = await SgiCore.UpdateFalla(Id, falla);
                DialogService.Close(falla);
            }
            catch (System.Exception sgiCoreUpdateFallaException)
            {
                    NotificationService.Notify(NotificationSeverity.Error, $"Error", $"Unable to update Falla");
            }
        }
