...
namespace Sgi.Pages
{
    public partial class AddPermissionComponent : ComponentBase
    {
        ...
        {
            permission = new Sgi.Models.SgiCore.Permission(){};
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Sgi.Models.SgiCore.Permission args)
        {
            try
            {
                var sgiCoreCreatePermissionResult = await SgiCore.CreatePermission(permission);
                DialogService.Close(permission);
            }
            catch (System.Exception sgiCoreCreatePermissionException)
            {
                    NotificationService.Notify(NotificationSeverity.Error, $"Error", $"Unable to create new Permission!");
            }
        }
